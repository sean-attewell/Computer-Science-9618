...
# A stack is like having a paper tray inbox on your desk. 
# Anytime a person walks by and drops a piece of paper or a letter in your inbox, it will go on the top of your inbox. 
...
# There are two common ways to implement a stack. One is by using a linked list, and the other is by using a dynamic array. 
# Both of these implementations work well.
...
# In the implementation that uses a linked list, the push method inserts a new node at the linked list's head, 
# and the pop method removes the node at the linked list's head.
...
# We looked at the call stack before when discussing recursion - function calls and execution contexts are managed on a call stack. 
# When you call a function, it's added to the call stack. 
# When it returns, it gets popped off of the stack. 

# Stacks can be useful in any situation where you desire a LIFO order. 
# One common use-case is for parsing strings. 
# Let's say you wanted to parse a string to ensure that all the parentheses in your string are correctly nested. 
# A stack could be useful for this. 
# When you encounter an opening parenthesis, you add it to the stack. 
# When you encounter a closing parenthesis, you remove the top opening parenthesis from the stack.
...
# Last, an iterative depth-first-search can be done using a stack. (ignore this line)
